...
for part in _sword _axe _pickaxe _shovel _hoe; do
  if [ "$2" = "1" ]; then
    ...
    cat templates/items$part.json | sed s/\$ITEM_NAME/${METAL_NAME}/g > $BP/items/tools/${METAL_NAME}$part.json
  else
...
for part in _sword _axe _pickaxe _shovel _hoe; do
  if [ "$2" = "1" ]; then
    ...
    cat templates/recipes$part.json | sed s/\$ITEM_NAME/${METAL_NAME}/g > $BP/recipes/tools/${METAL_NAME}$part.json
  else
