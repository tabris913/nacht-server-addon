 ...
 * @returns
 */
const gatherSlots = (player: Entity, item: string) =>
  Array(36)
    .fill(null)
    .map((_, index) => {
      try {
 ...
 * @returns
 */
export const countItem = (player: Entity, item: string) => {
  try {
 ...
 * @returns
 */
export const giveItem = (player: Entity, item: string, amount: number = 1) => {
  try {
    player
