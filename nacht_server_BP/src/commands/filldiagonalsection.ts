      ...
      Array.from({ length: hight }).forEach((_, index) => {
        blockVolumes.push(
          new BlockVolume(
            ...
            {
              x:
                from.x - width + 1 + (expand === ExpandMode.Expand ? -index : expand === ExpandMode.Shrink ? index : 0),
              y: from.y + (verticalDirection === VerticalDirection.UP ? index : -index),
              z: from.z - index,
            }
          )
        ...
        blockVolumes.push(
          new BlockVolume(
            ...
            },
            {
              ...
              y: from.y + (verticalDirection === VerticalDirection.UP ? index : -index),
              z:
                from.z + width - 1 + (expand === ExpandMode.Expand ? index : expand === ExpandMode.Shrink ? -index : 0),
            }
          )
        );
      });
