...
export default () =>
  system.beforeEvents.startup.subscribe((event) =>
    event.customCommandRegistry.registerCommand(
      ...
      ({ sourceEntity }, from: Vector3, to: Vector3, seconds: number = 1, interval = TicksPerSecond / 2) => {
        try {
          if (seconds <= 0) {
            return {
              ...
              while (count--) {
                edges.forEach(([start, end]) =>
                  getLocationBetween(start, end).forEach((loc) =>
                    sourceEntity.dimension.spawnParticle('minecraft:small_flame_particle', loc)
                  )
                );
