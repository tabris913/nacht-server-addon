      ...
      try {
        ...
        Logger.error(
          `Failed to gather block locations [town e-w] (${
            Math.abs(Math.max(northWest.z, -6401) - Math.min(southEast.z, 6401)) + 1
          }).`
        );
      ...
      try {
        ...
        Logger.error(
          `Failed to gather block locations [town n-s] (${
            Math.abs(Math.max(northWest.x, -6401) - Math.min(southEast.x, 6401)) + 1
          }).`
        );
    ...
    bases
      ...
      .forEach((baseBV) => {
        ...
        if (northWest.z <= southZ && southZ <= southEast.z) {
          const xArray = LocationUtils.makeArray(Math.max(northWest.x, westX), Math.min(southEast.x, eastX)).filter(
            (x) => x % 5 === 0
          );
