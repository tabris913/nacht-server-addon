...
const commandProcess = (origin, dice) => {
    ...
    const formatted = rolls.map((value) => surface === 100 && (value <= 5 || 95 < value)
        ? `${Formatting.Color.RED}${value.toString()}${Formatting.Reset}`
        : value.toString());
    system.runTimeout(() => world.sendMessage(`[${player.nameTag}] ${dice} = ${rolls.reduce((prev, cur) => prev + cur, 0)} ( ${formatted.join(' ')} )`), 1);
