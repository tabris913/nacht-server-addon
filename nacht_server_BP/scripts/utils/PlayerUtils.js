import { Player, PlayerPermissionLevel, world } from '@minecraft/server';
...
const getOperators = () => {
    try {
        return world
            .getPlayers()
            .filter((player) => player.playerPermissionLevel === PlayerPermissionLevel.Operator || player.hasTag(TAG_OPERATOR));
    }
    catch (error) {
